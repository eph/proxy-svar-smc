...
for h in range(49):
    # growth rates
    var_data['DIP_H'+str(h)] = c*(var_data[key['IP']] - var_data[key['IP']].shift(h+1)).shift(-(h+1)).shift(1)  / (h+1)
    var_data['DP_H'+str(h)] = c*(var_data[key['P']] - var_data[key['P']].shift(h+1)).shift(-(h+1)).shift(1)  / (h+1)
